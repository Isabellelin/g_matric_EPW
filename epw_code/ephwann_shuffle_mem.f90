  ...
  USE elph2,         ONLY : cu, cuq, lwin, lwinq, map_rebal, map_rebal_inv,     &
                            ...
                            sigmai_all, sigmai_mode,sigmai_mode_detail, gamma_all, epsi, zstar,    &
  ...
  ELSE ! (iterative_bte .AND. epmatkqread)
    ...
    IF (elecselfen .OR. plselfen) THEN
      ...
      IF (iverbosity == 3) THEN
        ...
        ALLOCATE(sigmai_mode_detail(nbndfst, nbndfst,nmodes, nktotf, nstemp), STAT = ierr)
        IF (ierr /= 0) CALL errore('ephwann_shuffle_mem', 'Error allocating sigmai_mode_detail', 1)
        sigmai_mode_detail(:, :, :, :,:) = zero
    ...
    IF (elecselfen .OR. plselfen) THEN
      ...
      IF (iverbosity == 3) THEN
        ...
        DEALLOCATE(sigmai_mode_detail, STAT = ierr)
        IF (ierr /= 0) CALL errore('ephwann_shuffle_mem', 'Error deallocating sigmai_mode_detail', 1)
