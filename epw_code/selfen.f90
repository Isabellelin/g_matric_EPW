  ...
  CONTAINS
    ...
    USE io_var,        ONLY : linewidth_elself,linewidth_elself_detail
    ...
    USE elph2,         ONLY : etf, ibndmin, ibndmax, nkqf, xqf, eta, nbndfst, &
                              ...
                              sigmar_all, sigmai_all, sigmai_mode,sigmai_mode_detail, zi_all, efnew, &
    ...
    CHARACTER(LEN = 256) :: fileselfen_detail
    ...
    IF (iqq == totq) THEN
      ...
      DO itemp = 1, nstemp
        ...
        IF (mpime == ionode_id) THEN
          ...
          fileselfen_detail = 'linewidth.elself_detail.' // trim(adjustl(tp)) //'K'
          OPEN(unit=linewidth_elself_detail,file=fileselfen_detail)  ! *******************************************
          WRITE(linewidth_elself, '(a)') '# Electron linewidth = 2*Im(Sigma) (meV)'
          IF (iverbosity == 3) THEN
            ! ********************************************************   linewidth_elself
            WRITE(linewidth_elself, '(a)', ADVANCE = 'no') '#      totol_knum       bands     modes:'
            WRITE(linewidth_elself, '(i9, 2x)', ADVANCE = 'no') nktotf
            WRITE(linewidth_elself, '(i9, 2x)', ADVANCE = 'no') nbndfst
            WRITE(linewidth_elself, '(i9, 2x)') nmodes
            WRITE(linewidth_elself, '(a)') '#      ik       ibnd                 E(ibnd)      imode          Im(Sigma)(meV)'
            ! **********************************************************
            WRITE(linewidth_elself_detail, '(a)', ADVANCE = 'no') '#      tks       bands     modes: '
            WRITE(linewidth_elself_detail, '(i9, 2x)', ADVANCE = 'no') nktotf
            WRITE(linewidth_elself_detail, '(i9, 2x)', ADVANCE = 'no') nbndfst
            WRITE(linewidth_elself_detail, '(i9, 2x)') nmodes
            WRITE(linewidth_elself_detail, '(a)') '#      ik       ibnd        jbnd         E(ibnd)     Ejbnd  imode          Im(Sgima)(meV)'
          ELSE
            WRITE(linewidth_elself, '(a)') '#      ik       ibnd           jbns      E(ibnd)      Im(Sigma)(meV)'
          ENDIF
          !
          DO ik = 1, nktotf
            !
            ...
            DO ibnd = 1, nbndfst
              ...
              IF (iverbosity == 3) THEN
                DO imode = 1, nmodes
                  ...
                  WRITE(linewidth_elself, '(E22.10, 2x)', ADVANCE = 'no') ryd2ev * ekk
                  WRITE(linewidth_elself, '(i9, 2x)', ADVANCE = 'no') imode
                  WRITE(linewidth_elself, '(f22.6, 2x)') ryd2mev * sigmai_mode(ibnd, imode, ik, itemp)
                  tt_ibnd = 0.0
                  Do jbnd = 1, nbndfst
                    !
                    ekq = etf_all(ibndmin-1+jbnd, ikq) - ef0
                    !
                    WRITE(linewidth_elself_detail,'(i9,2x)',advance='no') ik
                    WRITE(linewidth_elself_detail,'(i9,2x)',advance='no') ibndmin-1+ibnd
                    WRITE(linewidth_elself_detail,'(i9,2x)',advance='no') ibndmin-1+jbnd
                    WRITE(linewidth_elself_detail,'(E22.10,2x)',advance='no') ryd2ev * ekk
                    WRITE(linewidth_elself_detail,'(E22.10,2x)',advance='no') ryd2ev * ekq
                    WRITE(linewidth_elself_detail,'(i9,2x)',advance='no') imode
                    WRITE(linewidth_elself_detail,'(f22.6,2x)') ryd2mev*sigmai_mode_detail(ibnd,jbnd,imode,ik,itemp)
                    tt_ibnd = tt_ibnd + ryd2mev*sigmai_mode_detail(ibnd,jbnd,imode,ik,itemp)
                  ENDDO
                  WRITE(linewidth_elself_detail,'(a)',advance='no') '#************  '
                  WRITE(linewidth_elself_detail,'(f22.6,2x)') tt_ibnd
                ENDDO
              ELSE
                WRITE(linewidth_elself, '(i9, 2x)', ADVANCE = 'no') ik
                WRITE(linewidth_elself, '(i9, 2x)', ADVANCE = 'no') ibndmin - 1 + ibnd
                WRITE(linewidth_elself, '(E22.10, 2x)', ADVANCE = 'no') ryd2ev * ekk
                WRITE(linewidth_elself, '(E22.10, 2x)') ryd2mev * sigmai_all(ibnd, ik, itemp)
              ENDIF
              !
            ENDDO
            WRITE(stdout, '(5x, a/)') REPEAT('-', 67)
