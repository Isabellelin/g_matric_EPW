# -*- coding: utf-8 -*-
"""
Created on Mon Jan  4 10:52:59 2021
...
class dyn(object):
    def __init__(self,prefix,idyn):
        ...
        #for i in range(5): line = f.readline();print ('================',line)
        while True:
            line = f.readline()
            if ' ****************' in line:
                break
...
# get the prefix used for PH calculation
def get_prefix_from_ph():
    fr=open('ph.in','r').readlines()
    for line in fr:
        if 'prefix' in line:
            star_str = "'"
            end_str  = "'"
            return txt_wrap_by(star_str,end_str,line)
        else:
            continue
    pass
...
prefix = get_prefix_from_ph()#str(user_input)
